...
s32 is_collide_with_vehicle(f32 vehicleX, f32 vehicleY, f32 vehicleVelocityX, f32 vehicleVelocityZ, f32 distanceX,
...
void yoshi_valley_cpu_path(s32 playerId) {
    ...
    if ((previous == 0) && (bInMultiPathSection[playerId] == 1)) {
        cpu_enteringPathIntersection[playerId] = 1;
    }
    if ((previous == 1) && (bInMultiPathSection[playerId] == 0)) {
...
void update_player_path_completion(s32 playerId, Player* player) {
    ...
    if ((gCurrentCourseId == COURSE_YOSHI_VALLEY) && (gIsPlayerNewPathPoint == 1)) {
...
Path2D tmpPaths[2048];
TrackPathPoint gtp[4][2048];
TrackPathPoint gtlp[4][2048];
TrackPathPoint gtrp[4][2048];
s16 gtst[4][2048];
s16 gper[4][2048];
s16 gtccc[4][2048];
...
void init_course_path_point(void) {
    ...
    gVehicle2DPathPoint = tmpPaths;

    // Podium ceremony appears to allocate 1 * 8 bytes of data. Which would be aligned to 0x10.
    for (i = 0; i < 4; i++) {
        gTrackPaths[i] = &gtp[i][0];
		//get_next_available_memory_addr(gSizePath[i] * sizeof(TrackPathPoint));
        gTrackLeftPaths[i] =  &gtlp[i][0];
		//get_next_available_memory_addr(gSizePath[i] * sizeof(TrackPathPoint));
        gTrackRightPaths[i] = &gtrp[i][0];
		//get_next_available_memory_addr(gSizePath[i] * sizeof(TrackPathPoint));
        gTrackSectionTypes[i] = &gtst[i][0];
		//get_next_available_memory_addr(gSizePath[i] * sizeof(s16));
        gPathExpectedRotation[i] = &gper[i][0];
		//get_next_available_memory_addr(gSizePath[i] * sizeof(s16));
        gTrackConsecutiveCurveCounts[i] = &gtccc[i][0];
		//get_next_available_memory_addr(gSizePath[i] * sizeof(s16));
    }
...
void func_80016494(Camera* camera, UNUSED Player* unusedPlayer, UNUSED s32 arg2, s32 cameraId) {
    ...
    D_80163238 = playerId;
    pathIndex = gPathIndexByPlayerId[playerId];
    /* not a second `player += playerId` - upstream's restored code
     * double-increments and follows the wrong racer (OOB for id >= 4) */
    gNearestPathPointByCameraId[cameraId] =
        func_8000D33C(camera->pos[0], camera->pos[1], camera->pos[2], gNearestPathPointByCameraId[cameraId], pathIndex);
...
void func_800178F4(Camera* camera, UNUSED Player* unusedPlayer, UNUSED s32 arg2, s32 cameraId) {
    ...
    D_80163238 = playerId;
    pathIndex = gPathIndexByPlayerId[playerId];
    /* no second `player += playerId` (see above) */
...
void func_800188F4(Camera* camera, UNUSED Player* unusePlayer, UNUSED s32 arg2, s32 cameraId) {
    ...
    D_80163238 = playerId;
    /* no second `player += playerId` - upstream's restored code
     * double-increments and follows the wrong racer (OOB for id >= 4) */
    gNearestPathPointByCameraId[cameraId] =
        func_8000D33C(camera->pos[0], camera->pos[1], camera->pos[2], gNearestPathPointByCameraId[cameraId], pathIndex);
    playerPathPoint = gNearestPathPointByPlayerId[playerId];
...
void cpu_use_item_strategy(s32 playerId) {
    ...
    TrackPathPoint* pathPoint;
    uint8_t isValidBanana1;
    uint8_t isValidBanana2; 
    ...
    switch (cpuStrategy->branch) {
        ...
        case CPU_STRATEGY_WAIT_INIT_BANANA_BUNCH:
            ...
            if (BANANA_BUNCH_ACTOR(actor)->state == 6) {
                ...
                if ((BANANA_BUNCH_ACTOR(actor)->type != ACTOR_BANANA_BUNCH) || (isValidBanana2 == 0)) {
