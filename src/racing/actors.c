...
void render_shadow_for_tree(Vec3f pos, Vec3s rot, f32 scale) {
    Mat4 sp20;
    pos[1] += 2.0f;
...
void func_802979F8(struct Actor* arg0, UNUSED f32 arg1) {
    ...
    if (arg0->unk30.unk34 != 0) {
        ...
        render_shadow_for_tree(pos, rot, 0.45f);
    }
}
...
void render_actor_shell(Camera* camera, Mat4 matrix, struct ShellActor* shell) {
    ...
    // this could be improved by checking for:
    // 1) this is not blue shell
    // 2) the shell TLUT is the same as the last time this was called
    // in which case, don't invalidate again
...
extern void finish_actor_banana();
extern void setup_actor_banana_flat();
extern void finish_actor_banana_flat();
